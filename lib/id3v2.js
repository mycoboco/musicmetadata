...
module.exports = function (stream, callback, done, readDuration, fileSize) {
  ...
  strtok.parse(stream, function (v, cb) {
    ...
    switch (cb.state) {
      ...
      case 1.5:
        var shiftedBuffer = new Uint8Array(4)
        shiftedBuffer.set(cb.frameFragment.slice(1))
        shiftedBuffer.set(v, 3)
        v = shiftedBuffer
        cb.state = 2
...
function readFrameHeader (v, majorVer) {
  ...
  switch (majorVer) {
    case 2:
      header.id = String.fromCharCode.apply(null, v.slice(0, 3))
      header.length = common.strtokUINT24_BE.get(v, 3, 6)
    break
    case 3:
      header.id = String.fromCharCode.apply(null, v.slice(0, 4))
