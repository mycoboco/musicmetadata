...
function decodeBase64Bytes (buf) {
    var iof = buf.indexOf(0x3d) // =
    var len = (iof > 0) ? iof : buf.length
    var decoded = new Uint8Array(len)
    var j = 0;

    for (var i = 0; i < buf.length; i += 4) {
        var enc1 = codes.indexOf(buf[i])
        var enc2 = codes.indexOf(buf[i + 1])
        var enc3 = codes.indexOf(buf[i + 2])
        var enc4 = codes.indexOf(buf[i + 3])
        ...
        decoded[j++] = chr1
        if (enc3 != 64) decoded[j++] = chr2
        if (enc4 != 64) decoded[j++] = chr3
    }
  ...
  strtok.parse(innerStream, function (v, cb) {
    ...
    switch (cb.state) {
      ...
      case 5: // comment
        ...
        var idx = v.indexOf(0x3d) // =
        var key = v.slice(0, idx)
        key = String.fromCharCode.apply(null, key).toUpperCase()
        var value = v.slice(idx + 1)

        if (key === 'METADATA_BLOCK_PICTURE') {
          var ee = decodeBase64Bytes(value)
          value = common.readVorbisPicture(ee)
          callback(key, value)
        } else {
          callback(key, String.fromCharCode.apply(null, value))
        }
