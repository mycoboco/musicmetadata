...
var eq = exports.eq = function (a, b) {
...
exports.getParserForMediaType = function (types, header) {
  ...
  types.forEach(function (type) {
    ...
    if (header.length >= offset + type.buf.length) {
      ...
      var good = eq(type.buf, header.slice(offset, offset + type.buf.length))
      if (good === true) {
        parser = type.tag
      }
    }
...
exports.readVorbisPicture = function (buffer) {
  ...
  picture.type = PICTURE_TYPE[dv.getUint32(0)]

  var mimeLen = dv.getUint32(offset += 4)
  picture.format = String.fromCharCode.apply(null, buffer.slice(offset += 4, offset + mimeLen))

  var descLen = dv.getUint32(offset += mimeLen)
  picture.description = String.fromCharCode.apply(null, buffer.slice(offset += 4, offset + descLen))

  picture.width = dv.getUint32(offset += descLen)
  picture.height = dv.getUint32(offset += 4)
  picture.colour_depth = dv.getUint32(offset += 4)
  picture.indexed_color = dv.getUint32(offset += 4)

  var picDataLen = dv.getUint32(offset += 4)
